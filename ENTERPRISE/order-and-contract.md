...
4. **Write a quote** only after those assumptions are explicit. Follow [`quote-draft-outline.md`](quote-draft-outline.md). The quote is a document, not a checkout button. If the request cannot be quoted honestly, follow [`decline-or-defer.md`](decline-or-defer.md) instead of inventing a price.
...
**Does an interface-specified card mean the model is ready to license as weights?** No. Cards 02–23 document interfaces. Weights, indexes, local LLM runtimes, packed KV layouts, mesh radios, flash drivers, coil firmware, tesla fits, piezo/TENG drivers, joule-per-cycle fits, photodiode drivers, lux-to-joule fits, TPM bindings, measured hash joules, on-device schema compilers, measured parse joules, radio drivers, measured hop joules, survival-curve fits, energy-neutral certificates, trained NILM trees, trained forests, trained cascades, context-window certificates, and joule measurements are separate scoped work.
