...
Interface-specified cards (currently 02–12) are design contracts, not finished binaries. Model 11’s host allocator is a local partitioner, not a licensed energy market or mesh stack. Model 12’s checkpoint is host JSON, not flash.
...
**Does an interface-specified card mean the model is ready to license as weights?** No. Cards 02–12 document interfaces. Weights, indexes, local LLM runtimes, mesh radios, flash drivers, and joule measurements are separate scoped work.
