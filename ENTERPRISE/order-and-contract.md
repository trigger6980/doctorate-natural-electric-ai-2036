...
5. Upon agreement and execution of the contract, the requested artifacts are delivered. The packet shape is listed in [`post-quote-packet.md`](post-quote-packet.md). Later scope changes follow [`change-order.md`](change-order.md). Closeout follows [`engagement-closeout.md`](engagement-closeout.md). Keep vs discard after closeout follows [`records-retention.md`](records-retention.md). Factual errors after retention follow [`correction-notice.md`](correction-notice.md). New scope after closeout follows [`reopen-after-close.md`](reopen-after-close.md). An inquiry that cannot be quoted honestly follows [`decline-or-defer.md`](decline-or-defer.md). A question already answered by the public catalog follows [`handoff-to-public-tree.md`](handoff-to-public-tree.md).

Use [`inquiry-template.md`](inquiry-template.md) as the paste-in form. An inquiry is complete enough to discuss only when the six items in [`inquiry-completeness.md`](inquiry-completeness.md) are present. After that gate, a written quote should follow [`quote-draft-outline.md`](quote-draft-outline.md). After written acceptance, the packet follows [`post-quote-packet.md`](post-quote-packet.md). After a packet exists, changes follow [`change-order.md`](change-order.md). After packet and change orders settle, closeout follows [`engagement-closeout.md`](engagement-closeout.md). After closeout, records follow [`records-retention.md`](records-retention.md). After retention, factual corrections follow [`correction-notice.md`](correction-notice.md). After closeout, new work follows [`reopen-after-close.md`](reopen-after-close.md). Process questions that are not model-specific live in [`engagement-faq.md`](engagement-faq.md). License-object definitions live in [`licensing-boundary.md`](licensing-boundary.md). Cover-letter claim limits live in [`well-being-alignment.md`](well-being-alignment.md). Decline or defer language lives in [`decline-or-defer.md`](decline-or-defer.md). Public-tree handoff language lives in [`handoff-to-public-tree.md`](handoff-to-public-tree.md).
...
4. **Write a quote** only after those assumptions are explicit. Follow [`quote-draft-outline.md`](quote-draft-outline.md). The quote is a document, not a checkout button. If the request cannot be quoted honestly, follow [`decline-or-defer.md`](decline-or-defer.md) instead of inventing a price. If the public tree already answers, follow [`handoff-to-public-tree.md`](handoff-to-public-tree.md).
...
10. **Reopen after closeout.** New models, sites, or measurement campaigns follow [`reopen-after-close.md`](reopen-after-close.md) and re-run the completeness gate.
...
**What if I only want the public cards?** Use the repository. An inquiry is for identical/custom *delivery under contract*, not for reading the catalog. If that is all you needed, the maintainer may follow [`handoff-to-public-tree.md`](handoff-to-public-tree.md).
...
**Does an interface-specified card mean the model is ready to license as weights?** No. Cards 02–24 document interfaces. Weights, indexes, local LLM runtimes, packed KV layouts, draft models, mesh radios, flash drivers, coil firmware, tesla fits, piezo/TENG drivers, joule-per-cycle fits, photodiode drivers, lux-to-joule fits, TPM bindings, measured hash joules, on-device schema compilers, measured parse joules, radio drivers, measured hop joules, survival-curve fits, energy-neutral certificates, trained NILM trees, trained forests, trained cascades, context-window certificates, accept-rate certificates, and joule measurements are separate scoped work.
...
**Does Model 22 include a trained cascade or inference certificate?** No. The public card is a multi-stage exit-gate interface. Trained stage weights, camera traces, and certified detection are custom scope.

**Does Model 23 include a packed KV cache or context-window certificate?** No. The public card is a cache-budget gate interface. Packed layouts, quantized KV, and certified long-context runtimes are custom scope.

**Does Model 24 include a draft model or accept-rate certificate?** No. The public card is a draft-plus-verify gate interface. Draft weights, tree attention, and certified speculative-decode runtimes are custom scope.
...
**What if we want more work after closeout?** Use [`reopen-after-close.md`](reopen-after-close.md). That is a new inquiry, not a correction and not a silent change order.

**What if the inquiry cannot be quoted honestly?** Use [`decline-or-defer.md`](decline-or-defer.md). A decline is not a packet and not a waitlist position.

**What if I only needed a public file?** Use [`handoff-to-public-tree.md`](handoff-to-public-tree.md). A handoff is not a packet and not a decline with prejudice.

Process-only questions (identical vs custom, complete-inquiry contents, allowed cover-letter claims) are expanded in [`engagement-faq.md`](engagement-faq.md) and [`inquiry-completeness.md`](inquiry-completeness.md). License-object definitions are in [`licensing-boundary.md`](licensing-boundary.md). Mission language limits are in [`well-being-alignment.md`](well-being-alignment.md). Quote section order is in [`quote-draft-outline.md`](quote-draft-outline.md). Maintainer paste-back text is in [`inquiry-response-template.md`](inquiry-response-template.md). Packet contents after acceptance are in [`post-quote-packet.md`](post-quote-packet.md). Scope changes after a packet are in [`change-order.md`](change-order.md). Closeout after settled changes is in [`engagement-closeout.md`](engagement-closeout.md). Keep vs discard after closeout is in [`records-retention.md`](records-retention.md). Factual corrections after retention are in [`correction-notice.md`](correction-notice.md). New scope after closeout is in [`reopen-after-close.md`](reopen-after-close.md). Decline or defer is in [`decline-or-defer.md`](decline-or-defer.md). Public-tree handoff is in [`handoff-to-public-tree.md`](handoff-to-public-tree.md).
