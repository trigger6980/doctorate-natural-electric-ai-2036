...
`stamp_invariants(inquiry)` bundles those checks: action consistency, exactly-one flag, partition disjoint, outline coverage *or* idle-not-ready, lane-count sum, price-verb block, commercial-figure blank, and commercial-lane sealed. Stamp fields: `stamp_invariants_ok`, `stamp_invariants`. A true packet is a host coherence check. It is not a published dollar amount and it does not add a `publish_price` verb.
