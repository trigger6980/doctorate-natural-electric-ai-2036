...
`stamp_invariants(inquiry)` bundles those checks: action consistency, exactly-one flag, partition disjoint, outline coverage *or* idle-not-ready, lane-count sum, price-verb block, commercial-figure blank, commercial-lane sealed, and lane-fill alignment. Stamp fields: `stamp_invariants_ok`, `stamp_invariants`. A true packet is a host coherence check. It is not a published dollar amount and it does not add a `publish_price` verb.
...
`commercial_figure_blank(inquiry)` is the on-repo blank for heading `commercial_figure_off_repo`. `on_repo_value` is always `None`. `fill_on_repo` for that key stays false. When the outline is ready the key appears only in `off_repo_keys`. Stamp fields: `commercial_figure_blank`, `commercial_figure`. A true blank is not a published dollar amount.

`commercial_lane_sealed(inquiry)` is the lane lock for the same heading. When the outline is ready the lane is `off_repo`; when idle it is `not_ready`. The lane is never `public_fill`. Stamp fields: `commercial_lane_sealed`, `commercial_lane`. A sealed lane is not a published dollar amount.

`lane_fill_aligned(inquiry)` is the fill-mask lock. `fill_on_repo[key]` is true only when that heading's lane is `public_fill`. The commercial heading stays unfilled. `mismatches` must stay empty. Stamp fields: `lane_fill_aligned`, `lane_fill`. A true alignment is not a published dollar amount.
...
- Treating `commercial_figure_blank = true` or `commercial_figure.on_repo_value = None` as a published dollar amount.
- Treating `commercial_lane_sealed = true` or `commercial_lane.lane = off_repo` as a published dollar amount.
- Treating `lane_fill_aligned = true` or an empty `lane_fill.mismatches` list as a published dollar amount.
...
- That `commercial_figure_blank` or `commercial_figure` is a checkout form.
- That `commercial_lane_sealed` or `commercial_lane` is a checkout form.
- That `lane_fill_aligned` or `lane_fill` is a checkout form.
