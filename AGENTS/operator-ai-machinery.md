...
9. **Enterprise Interface** — configuration and licensing surface for identical or customized deployments. Inquiry helper refuses observer tokens and refused `intended_use` values with the same `observer_not_evidence` token.
10. **Host sandbox** — `SANDBOX/run_prototypes.py` runs the public stubs as specialized agents on a host. Observer demo writes stamped claim labels. Not hardware.
