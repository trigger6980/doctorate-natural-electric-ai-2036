...
9. **Enterprise Interface** — configuration and licensing surface for identical or customized deployments. Inquiry helper refuses observer tokens and refused `intended_use` values with the same `observer_not_evidence` token. `stamp(inquiry)` is a completeness snapshot only.
