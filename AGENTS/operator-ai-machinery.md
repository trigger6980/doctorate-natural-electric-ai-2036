...
2. **Claim gate** — `AGENTS/claim_gate.py` allows `host_log` / `sandbox_demo` and refuses `field_generation` / `quote_evidence` / `result_record` with `observer_not_evidence`. `stamp` / `stamp_many` attach those reasons to observer dicts for the sandbox. Not a legal opinion.
...
9. **Enterprise Interface** — configuration and licensing surface for identical or customized deployments. Inquiry helper refuses observer tokens and refused `intended_use` values with the same `observer_not_evidence` token. `stamp(inquiry)` is a completeness snapshot only. `public_fill_keys` lists the nine outline keys a maintainer may fill in the public tree; `off_repo_keys` lists the commercial-figure key when the outline is ready; `partition_keys` records that those lists stay disjoint and, when a draft is allowed, cover the ten outline headings. `section_lane` / `section_lanes` map each heading to `public_fill`, `off_repo`, or `not_ready`. `lane_counts` tallies those three lanes. `next_maintainer_action` is the filing verb (`decline` / `ask` / `copy_headings`). `action_flags` is the mutex on those verbs (`exactly_one` must stay true; `publish_price` stays false). `action_consistent` is true only when `next_action` equals the unique true flag. None of those fields is a price.
10. **Host sandbox** — `SANDBOX/run_prototypes.py` runs the public stubs as specialized agents on a host. Observer demo writes stamped claim labels. Inquiry demo writes `inquiry_stamp.json`. Not hardware.
