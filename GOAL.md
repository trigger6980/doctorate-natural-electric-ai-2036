...
- [ ] All artifacts verifiable and reproducible from the repository alone (tests exist for scheduler + voltage proxy + task-graph + policy gate + checkpoint files + listen inventory + energy broker + brokered executor + inquiry completeness + inquiry stamp + outline_ready + copy_headings + public_fill_keys + off_repo_keys + partition_keys + section_lane + lane_counts + next_maintainer_action + action_flags + action_consistent + stamp_invariants + price_verbs_blocked + commercial_figure_blank + commercial_lane_sealed + lane_fill_aligned + Model 49 gas gate + Model 49 gas preflight on the policy executor + sandbox `_gas_reason` demo + host energy observer stub + host claim gate + claim stamp / claim_scan + inquiry intended_use; hardware measurements still pending).
...
- [x] Host energy observer stub (`energy_observer.record_sample`) with `host_placeholder` / `hardware_pending` sources only; `measured` refused; not a field instrument.
- [x] Host claim gate (`claim_gate.refuse_reason`) allows `host_log` / `sandbox_demo` and refuses field / quote / result uses with `observer_not_evidence`.
- [x] Host claim stamp (`claim_gate.stamp` / `stamp_many`) attaches those reasons to sandbox observer output; not a field certificate.
...
- [x] Inquiry helper `public_fill_keys(inquiry)` plus stamp fields `public_fill_keys` (nine keys when ready; never includes `commercial_figure_off_repo`).
...
- [x] Inquiry helper `next_maintainer_action(inquiry)` plus stamp fields `next_action` / `next_action_copy_headings` (decline / ask / copy_headings; never publish_price).
- [x] Inquiry helper `action_flags(inquiry)` plus stamp fields `action_flags` / `action_exactly_one` (mutex on decline / ask / copy_headings; publish_price always false).
- [x] Inquiry helper `action_consistent(inquiry)` plus stamp fields `action_consistent` / `action_flag_verb` (verb matches unique true flag; publish_price stays false).
- [x] Inquiry helper `stamp_invariants(inquiry)` plus stamp fields `stamp_invariants_ok` / `stamp_invariants` (host coherence check; not a published dollar amount).
- [x] Inquiry helper `price_verbs_blocked(inquiry)` plus stamp fields `price_verbs_blocked` / `price_verbs` / top-level `covers_or_idle` (filing verbs only; blocked list includes publish_price; not a rate card).
- [x] Inquiry helper `commercial_figure_blank(inquiry)` plus stamp fields `commercial_figure_blank` / `commercial_figure` (`on_repo_value` always None; not a rate card).
- [x] Inquiry helper `commercial_lane_sealed(inquiry)` plus stamp fields `commercial_lane_sealed` / `commercial_lane` (commercial heading never `public_fill`; not a rate card).
- [x] Inquiry helper `lane_fill_aligned(inquiry)` plus stamp fields `lane_fill_aligned` / `lane_fill` (fill mask matches public_fill lanes only; not a rate card).
